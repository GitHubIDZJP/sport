...
- (void)cretBtnView {
    ...
    for (int i = 0; i < [self.titleArray count]; i++)  {
        ...
        CGFloat width = [self.titleArray[i] sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:self.btnFont]}].width+10;
       // CGFloat ScreenWidth = SCREEN_WIDTH ;//[[UIScreen mainScreen] bounds].size.width ;
        CGFloat jianJu = (SCREEN_WIDTH - titleWidth)/(self.titleArray.count +1);
...
- (void)CreatelineView {
    if (!_lineView) {
       ...
       // _lineView.backgroundColor = colorWithHexString(@"#4CB13B");//[UIColor colorWithRed:0.885 green:0.000 blue:0.039 alpha:1.000];
    }
}
...
- (void)btnClick:(UIButton *)sender {
    ...
    [UIView animateWithDuration:0.1 animations:^{
      //  [_seletedBtn setTitleColor:[UIColor colorWithRed:0.276 green:0.274 blue:0.277 alpha:1.000] forState:UIControlStateNormal];
        [_seletedBtn setTitleColor: colorWithHexString(@"#666666") forState:UIControlStateNormal];//不选择的文字颜色
        [sender setTitleColor:colorWithHexString(@"#4CB13B") forState:UIControlStateNormal];//选择后得文字颜色
