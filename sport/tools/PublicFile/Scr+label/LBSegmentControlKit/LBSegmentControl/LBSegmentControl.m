...
- (void)layoutSubviews {
    ...
    if (self.titles.count != 0) {
     ...
     #pragma mark 设置bottomView的frame
    // 设置底部条的宽度
    self.bottomView.width = self.currentItem.width/5;
    
    // 设置底部条x值
    self.bottomView.left = LBkScreen_width/11;
...
#pragma mark 调试移动下标的动画
    [UIView animateWithDuration:0.3 animations:^{
        ...
        // 设置动画增长或缩短宽度
        self.bottomView.width =  self.currentItem.width/5 + (differenceW * (1 - self.currentSelectProgress));
        // 两个标题项中x的差
        CGFloat differenceX = self.nextItem.left - self.currentItem.left +20 ;
        // 设置动画修改x值
        self.bottomView.left =  self.currentItem.left + (differenceX * (1 - self.currentSelectProgress)+20);
