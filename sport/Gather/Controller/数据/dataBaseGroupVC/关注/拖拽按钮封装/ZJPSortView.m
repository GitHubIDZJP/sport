...
#define DEFAULT_COLUMN_MARGIN 17 //默认列边距
#define DEFAULT_COLUMN_COUNT 3  //默认列数量
...
{
    ZJPSortView * sortView = [[ZJPSortView alloc] init];
...
{
    ZJPSortView * sortView = [ZJPSortView sortView];
...
- (void)initailButtons
{
    ...
    for (NSInteger i = 0; i < _titlesArray.count; i++) {
        ...
        [movedBtn setBackgroundColor:colorWithHexString(@"#F3F6F4")];
        movedBtn.layer.cornerRadius = 5;
        [movedBtn setTitleColor:colorWithHexString(@"#333333") forState:(UIControlStateNormal)];
...
- (void)settingButtonFrame
{
    ...
    CGFloat buttonH = 34;
    CGFloat buttonW = 82;//(SELF_SIZE.width - _columnMargin * (_columnCount + 1)) / _columnCount * 1.0;
    CGFloat buttonX = 0;
    CGFloat buttonY = 0;
    // 设置按钮初始位置
    for (NSInteger i = 0; i < self.buttons.count; i++) {
        ...
        buttonX =0 + _columnMargin + column * (buttonW + _columnMargin);
        buttonY = (_columnMargin- 2) + row * (buttonH + _columnMargin-7);//-2是为了拉升第一行到顶边的距离,-7是为了每行之间的近距离缩小7
