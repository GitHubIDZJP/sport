...
#import "CBasePageController.h"




@interface CBasePageController ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource>
{
    ...
    CBasePageTitleButton *oldButton;
...
@implementation CBasePageController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.whiteColor;
    
    self.lineWidth = self.lineWidth>0?self.lineWidth:0;
    ...
    self.chooseColor = self.chooseColor?self.chooseColor:[UIColor clearColor];
...
-(void)setDataSource:(id<CBasePageControllerDataSource>)dataSource
{
...
-(void)reloadScrollPage
{
    if ([self.dataSource respondsToSelector:@selector(numberViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<totalVC; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexViewControllers:)]) {
                ...
                if ([self.dataSource respondsToSelector:@selector(viewPager:titleButtonStyle:)])
                {
                    ...
                    CBasePageTitleButton *autoButton = [[CBasePageTitleButton alloc] init];
        ...
        if (buttonList.count && ((UIButton *)buttonList.lastObject).frame.origin.x + ((UIButton *)buttonList.lastObject).frame.size.width<self.view.frame.size.width)
        {
            ...
            for (CBasePageTitleButton *button in buttonList) {
...
-(void)titleButtonClick:(CBasePageTitleButton *)sender
{
...
-(void)titleButtonConvert:(CBasePageTitleButton *)sender
{
    oldButton.selected = NO;
    sender.selected = YES;
    oldButton = sender;
...
@implementation CBasePageTitleButton
- (instancetype)init
{
    self = [super init];
    if (self) {
